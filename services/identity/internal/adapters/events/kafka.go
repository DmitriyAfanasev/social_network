// Package events содержит Kafka-адаптеры identity-сервиса.
package events
...
// Publisher публикует identity-события в общий Kafka topic.
type Publisher struct {
	writer *kafka.Writer
}

// NewPublisher создаёт publisher identity-событий.
...
func (p *Publisher) Publish(ctx context.Context, event ports.OutboxEvent) error {
	payload, err := json.Marshal(eventEnvelope{
		...
		Payload: json.RawMessage(event.Payload), CorrelationID: event.CorrelationID, CreatedAt: event.CreatedAt,
	})
	if err != nil {
		return err
	}
