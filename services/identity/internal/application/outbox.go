...
const identityOutboxBatchSize = 100

// OutboxPublisher публикует identity-события с повторными попытками.
type OutboxPublisher struct {
	outbox ports.OutboxRepository
	broker ports.EventPublisher
}

// NewOutboxPublisher создаёт publisher identity-событий.
...
func (p *OutboxPublisher) RunOnce(ctx context.Context) error {
	now := time.Now().UTC()
	events, err := p.outbox.Claim(ctx, identityOutboxBatchSize, now)
	if err != nil {
		return err
	}
	var firstErr error
	for _, event := range events {
		if err := p.broker.Publish(ctx, event); err != nil {
			...
			if markErr := p.outbox.MarkFailed(ctx, event.ID, now.Add(identityRetryDelay(event.Attempts)), err.Error()); markErr != nil && firstErr == nil {
				firstErr = markErr
			}
			continue
		}
...
func identityRetryDelay(attempts int) time.Duration {
	if attempts < 1 {
		attempts = 1
	}
	if attempts > 6 {
