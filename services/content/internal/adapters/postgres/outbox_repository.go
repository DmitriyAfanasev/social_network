...
// OutboxRepository реализует публикацию content-событий после commit.
type OutboxRepository struct {
	pool *pgxpool.Pool
}

// NewOutboxRepository создаёт адаптер content outbox.
func NewOutboxRepository(pool *pgxpool.Pool) *OutboxRepository {
	return &OutboxRepository{pool: pool}
}

// Claim атомарно резервирует пачку pending-событий.
func (r *OutboxRepository) Claim(ctx context.Context, limit int, now time.Time) ([]ports.OutboxEvent, error) {
	...
	const query = `
		WITH candidates AS (
			SELECT id FROM content.outbox_events
		...
		UPDATE content.outbox_events AS events
...
func (r *OutboxRepository) MarkPublished(ctx context.Context, eventID uuid.UUID, publishedAt time.Time) error {
	_, err := r.pool.Exec(ctx, `UPDATE content.outbox_events SET published_at = $2, locked_at = NULL, last_error = NULL WHERE id = $1 AND published_at IS NULL`, eventID, publishedAt)
	return err
}
...
func (r *OutboxRepository) MarkFailed(ctx context.Context, eventID uuid.UUID, nextAttempt time.Time, reason string) error {
	_, err := r.pool.Exec(ctx, `UPDATE content.outbox_events SET available_at = $2, locked_at = NULL, last_error = $3 WHERE id = $1 AND published_at IS NULL`, eventID, nextAttempt, reason)
	return err
}
