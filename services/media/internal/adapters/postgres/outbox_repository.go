...
func (r *OutboxRepository) Claim(ctx context.Context, limit int, now time.Time) ([]ports.OutboxEvent, error) {
	...
	const query = `
		WITH candidates AS (
			SELECT id FROM media.outbox_events
		...
		UPDATE media.outbox_events AS events
...
func (r *OutboxRepository) MarkPublished(ctx context.Context, eventID uuid.UUID, publishedAt time.Time) error {
	_, err := r.pool.Exec(ctx, `UPDATE media.outbox_events SET published_at = $2, locked_at = NULL, last_error = NULL WHERE id = $1 AND published_at IS NULL`, eventID, publishedAt)
	return err
}
...
func (r *OutboxRepository) MarkFailed(ctx context.Context, eventID uuid.UUID, nextAttempt time.Time, reason string) error {
	_, err := r.pool.Exec(ctx, `UPDATE media.outbox_events SET available_at = $2, locked_at = NULL, last_error = $3 WHERE id = $1 AND published_at IS NULL`, eventID, nextAttempt, reason)
	return err
}
