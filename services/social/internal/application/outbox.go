...
const socialOutboxBatchSize = 100

// OutboxPublisher публикует social-события с повторными попытками.
type OutboxPublisher struct {
	outbox ports.OutboxRepository
	broker ports.EventPublisher
}

// NewOutboxPublisher создаёт publisher social-событий.
...
func (p *OutboxPublisher) RunOnce(ctx context.Context) error {
	now := time.Now().UTC()
	events, err := p.outbox.Claim(ctx, socialOutboxBatchSize, now)
	if err != nil {
		return err
	}
	var firstErr error
	for _, event := range events {
		if err := p.broker.Publish(ctx, event); err != nil {
			...
			_ = p.outbox.MarkFailed(ctx, event.ID, now.Add(socialRetryDelay(event.Attempts)), err.Error())
			continue
		}
...
func socialRetryDelay(attempts int) time.Duration {
	if attempts < 1 {
		attempts = 1
	}
	if attempts > 6 {
